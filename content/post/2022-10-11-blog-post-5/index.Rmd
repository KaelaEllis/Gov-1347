...
gdp_q_final <- left_join(gdp_q7, gdp_q6, by = "year") %>% 
  ...
  rename(gdp_percent_difference = gdp_q7_q6) %>%  select(year, gdp_percent_difference) 
...
lm_incumb <- lm(dem_votes_major_percent ~ incumb, 
              data = incumbency)
lm_GDP <- lm(dem_votes_major_percent ~ gdp_percent_difference, data = gdp_q_final)
...
all <- full_join(ad_freq_new, incumbency,  by = "year")


lm_all <- lm(dem_votes_major_percent ~  ad_freq_new + incumbency, data = all)

model <- predict(lm_ads, ad_freq_new, interval="prediction")
mean(model)

```
My prediction including ads is Dems win 59%. However I reject the use of ads in forming my prediction because of its low R2, and the general limited ad information we have district by district. I therefore choose to return to my old prediction of Dems win 51% of the house vote.  
